...
class Elasticity(Model):
    ...
    def __init__(self, dim, domain, space, path, em, g, ds_g, dir_bcs, alpha, eps=1e-3):
    ...
    def adjoint(self, phi, U):
        ...
        Wf += self.djfunc(u, q) * self.dx + self.dlfunc(u, q)
    ...
    def derivative(self, phi, U, P):
        ...
        S1_J = (self.jfunc(u) + self._DW(F, grad(p))) * self.Id
        S1_J += Q1 + Q2
        S1_J *= self.A(phi)
