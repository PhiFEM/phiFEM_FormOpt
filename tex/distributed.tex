...
\newcommand{\pyfunc}[1]{\operatorname{\texttt{#1}}}
...
are chosen as
\begin{align}
    T  &= \frac{\text{\texttt{dfactor}}}{\mathcal{B}(\theta, \theta)}\quad\text{and} \label{eq:tend} \\
    \hat{s} &= (s_{\max} - s_{\min}) \left( \frac{T - t_{\min}}{t_{\max} - t_{\min}} \right)^{1/6} + s_{\min}, \label{eq:steps}
\end{align}
respectively, with \texttt{dfactor}, \texttt{lv\_time} $= (t_{\min}, t_{\max})$, and
\texttt{lv\_iter} $= (s_{\max}, s_{\max})$ user-chosen parameters.
Moreover, randomness can be introduced by specifying a \texttt{noise} level:
$T$ is scaled by a random factor drawn from the uniform distribution $\mathcal{U}(1 - \text{\texttt{noise}}, 1 + \text{\texttt{noise}})$
and $\hat{s}$ is replaced by a sample drawn from the normal distribution $\mathcal{N}(\hat{s}, (\text{\texttt{noise}} \cdot \hat{s})^2) $.
We consider a minimal value for $\mathcal{B}(\theta, \theta)$ to prevent division by zero in \eqref{eq:tend}.
Before return $T$ and $\hat{s}$, we ensure that $t_{\min}\leq T \leq t_{\max}$, $s_{\min}\leq \hat{s} \leq s_{\max}$,
and $\hat{s}$ be an integer.

\textbf{Stopping criterion.}
We consider the following stopping criterions:
\[
\left| (J(\Omega^i)-J(\Omega^j))_{j\in I(i)}  \right|_{\infty} < \text{\texttt{cost\_tol}} \left|J(\Omega^i)\right|
\]
for the problem without constraints;
\begin{align*}
&\left| (L(\Omega^i, z^i, \lambda^i, \mu^i)-L(\Omega^j, z^j, \lambda^j, \mu^j))_{j\in I(i)}  \right|_\infty \\
&< \text{\texttt{lgrn\_tol}} \left|L(\Omega^i, z^i, \lambda^i, \mu^i)\right|
\end{align*}
\[
\text{and}\quad\left|C(\Omega^i) - \textbf{1}\right|_\infty < \text{\texttt{ctrn\_tol}}
\]
for the problem with constraints,
where
$\left|\cdot\right|_{\infty}$ denotes the infinity norm
and $I(i) = \left\{i-\text{\texttt{prev}}, \dots,i-1\right\}$.
The parameters \pyarg{cost\_tol}, \pyarg{lgrn\_tol}, and \pyarg{ctrn\_tol} 
are error tolerances and \pyarg{prev} is the number of previous values considered.

\textbf{Initial guess.}
The initial level set function is constructed
using two arrays:  \pyarg{centers} and \pyarg{radii}.
The array \pyarg{centers} contains points
which represent centers of balls included in $\mathcal{D}$,
and the array \pyarg{radii} contains the corresponding radii.
Then, we define $\phi^0$ as follows:
\begin{equation}
	\phi^0(x)= \pyarg{factor} \cdot \max_k \omega_k(x),	
\end{equation}
where $\omega_k$ is given by
\begin{equation}
\omega_k(x) = \pyarg{raddi}[k] - \left|x - \pyarg{centers}[k]\right|_{\pyarg{ord}}. 
\end{equation}
...
Note that the set
\[
\Omega_k = \left\{x\mid \pyarg{factor} \cdot \omega_k(x) < 0\right\}
\]
determines either a ball or the complement of a ball,
if \pyarg{factor} is positive or negative, respectively.
Applying level set operations we have
\[
\Omega^0 =  \bigcap_{k}  \Omega_k =
\begin{cases}
  \mathcal{D} \setminus \bigcup_k \mathbb{B}(k) & \text{if } \pyarg{factor} > 0, \\
  \bigcup_k \mathbb{B}(k) & \text{if } \pyarg{factor} < 0.
\end{cases}
\]
where $\mathbb{B}(k)$ is the $\pyarg{ord}$-norm ball
centered at $\pyarg{centers}[k]$ with radius $\pyarg{raddi}[k]$.
Thus, the initial level set can be either $\mathcal{D}$ with holes
or the union of balls included in $\mathcal{D}$.
