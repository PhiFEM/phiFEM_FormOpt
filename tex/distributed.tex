...
%%%%%%%%%%%%%%%%%%%%%%%
\begin{algorithm}[H]
\caption{Basic Level Set Algorithm}
\small
\begin{algorithmic}[1]
\State \textbf{Initialization:} Choose an initial $\phi^0$.
\For{$k = 0, 1, 2, \dots$ until convergence}
	\State Define $\Omega^i = \left\{x\in\mathcal{D}\mid \phi^i(x)<0\right\}$.
	...
	\Statex \hspace{3em} Let $\theta$ be the solution.
	\State Solve the \textbf{transport} equation with $\theta$.
	\Statex \hspace{3em} Let $\phi^{i+1}$ be the solution.
\EndFor
\end{algorithmic}
\end{algorithm}
...
One for inverse problem in elasticity, one for heat equation, etc ... For each model, what changes is the PDEs, the geometric configuration such as boundary conditions and domain, and the tensors $S_1,S_0$.
...
It implements parallelism and uses the distributed
form of the shape derivative. The main classes included
in \code{distributed.py} are the following:

\begin{itemize}[left=0.0em]

\item \code{Model}: This is an abstract class for the problem equations.
...
\item \code{Velocity}: This class implements the numerical method
that solves the velocity equation. It uses the shape derivative
components defined on the model class.

\item \code{Level}: It provides the level set function at each
iteration by solving a diffusive version of the transport equation.
The main input is the velocity field.

\item \code{InitialLevel}: This class creates the initial
...
Data parallelism, Task parallelism, and a combination of these
called Mix parallelism.

Data parallelism refers to solve the problem in a mesh
that is distributed on several processes. All the weak formulations
are solved on the this distributed mesh.

In the Task parallelism paradigm each state equation is solved
in a processes, and then, the same applies to each adjoint equation.
It is necessary to have the a copy of the same mesh in each processes.
The velocity field and the level set function are computed
in the processes 0.

The Mix parallelism paradigm combines the previous ones by
solving each state equation in a mesh that is distributed in
a group of subprocesses. The adjoint equations are solving in the same 
