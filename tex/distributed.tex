...
\end{lstlisting}
Subclasses of \code{Model} (user-defined) must not override
...
\begin{lstlisting}
	...
	runMP(...)
\end{lstlisting}
We will see in Subsection \ref{sub:parallel} that the methods
\code{runDP}, \code{runTP}, and \code{runMP}
implement different modes of parallelism.
...
Its method \code{func} returns a callable function
that represents the initial level set function $\phi^0$.

\item \code{PPL}:
It implements the Lagrangian method developed in \cite{MR4589221}
...
The following are some help functions:
...
\item \code{create\_space}: It is wrapper to create a FEniCSx function space.

\item \code{homogeneous\_dirichlet}: This function
...
The main functions of \code{distributed.py}
implements Algorithm \ref{eq:algo_lv} along with
...
Associated to each test there is a folder in \code{results/}
where the results are saved.
In \code{load.py} and \code{plots.py} there are secondary codes
to plot the results. Since the results are written in Xdmf format,
we recommend Paraview to see them.

Several classes and functions are used internally and the user does not need
to know how to use them. Below we describe the stages the user must follow,
along with the classes and function that it must use:
\begin{enumerate}
	\item Creation of a model class (subclass of class \code{Model}) with the problem equations. 
	\item Definition of the domain, space, and boundary conditions. To facilitate this part,
	we have written \code{create\_domain\_2d\_<DP|TP|MP>} and many other functions
	to set the boundary conditions. 
	\item Model initialization and execution by calling the method \code{run<DP|TP|MP>}.
\end{enumerate}
...
We must start by creating a subclass of \code{Model} class
