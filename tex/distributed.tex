...
\lstset{
	...
	emph={create_initial_level, save_initial_level, pde, adjoint, cost, constraint, derivative, bilinear_form, runDP, runTP, runMP},
...
It serves as a base for user-defined classes, which must contain
...
the bilinear form for the velocity---written exclusively using
...
\end{lstlisting}
Subclasses of \code{Model} (i.e., user-defined classes)
must \textbf{not} override the following methods:
\begin{lstlisting}
	...
	runMP(...)
\end{lstlisting}
As we will see in Subsection \ref{sub:parallel}, the methods
\code{runDP}, \code{runTP}, and \code{runMP}
implement different parallelism modes.
...
the linear system (see the \code{run} method). 

\item \code{Level}:
This class provides the level set function at each iteration
by solving a diffusive version of the transport equation.
...
In \code{run}, before starting the time iterations, the right-hand side is compiled,
...
\begin{itemize}[left=0.0em]
\item \code{runDP} for data parallelism.
\item \code{runTP} for task parallelism.
\item \code{runMP} for mixed parallelism.
\end{itemize}
The user-defined subclasses inherit these functions from the \code{Model} class.

In addition to \code{distributed.py}, we have written
several problem models in \code{models.py}:
\begin{itemize}[left=0.0em]
\item \code{Compliance}: compliance minimization;
\item \code{CompliancePlus}: similar to \code{Compliance}, but using multiple load cases;
...
along with the classes and function that must be used:
